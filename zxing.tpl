<template id="n-form-zxing">
	<div class="n-form-zxing n-form-scan n-form-component" :class="{'n-form-zxing-scanned': scanned, 'n-form-scanned': scanned}">
		...
		<div v-if="switchCode && code" class="n-form-scan-code" :style="{'width':width + 'px', 'height': height + 'px'}">
			...
			<div v-show="!scanning" class="actions"><button class="n-form-zxing-retry n-form-scan-retry" @click="rescan"><span class="icon" :class="icon" v-if="icon"></span><span class="label">{{ buttonLabel ? buttonLabel : "%{Scan Code}" }}</span></button></div>
		</div>
