<template id="n-form-zxing">
	<div class="is-form-zxing n-form-component" :class="{'is-zxing-scanned': scanned, 'is-scanned': scanned}">
		...
		<div class="n-form-zxing-container n-form-scan-container" v-show="!switchCode || !code">
			...
			<div ref="overlay" class="overlay" v-show="!scanning"><button :class="buttonClass" class="is-button n-form-zxing-retry n-form-scan-retry" @click="rescan"><span class="icon" :class="icon" v-if="icon"></span><span class="label">{{ buttonLabel ? buttonLabel : "%{Scan Code}" }}</span></button></div>
		</div>
		<div v-if="switchCode && code" class="n-form-barcode" :style="{'width':width + 'px', 'height': height + 'px'}">
			...
			<div v-show="!scanning" class="actions"><button :class="buttonClass" class="is-button n-form-barcode-retry" @click="rescan"><span class="icon" :class="icon" v-if="icon"></span><span class="label">{{ buttonLabel ? buttonLabel : "%{Scan Code}" }}</span></button></div>
		</div>
